...
In rapid-fulfillment dark stores and sorting hubs (such as Zepto TOAT operations), pickers, sorters, and packers repeatedly scan hundreds of tote bins, rack bays, and shelf locations every shift.
...
- **Interruptive UIs**: Annoying toasts, confirm dialogs, and extra buttons cost seconds on every single bin.

### The Solution
**TOAT QR Scanner** is a purpose-built, single-screen utility designed specifically for warehouse staff:
1. **Fixed CPLM- Prefix**: Locked into the interface. Workers only type the suffix (such as `a18` or `a1b15`).
...
│                                ↓                                 │
│ 4. QR code renders immediately (Haptic pulse)                    │
│                                ↓                                 │
...
|---|---|---|
| **UI Framework** | React 19 | Component architecture, state hooks (`useState`, `useEffect`, `useRef`) |
| **Build Engine** | Vite 8 | Instant HMR, rapid production bundling (`< 300ms`) |
| **QR Generator** | qrcode.react | Canvas-based high-speed client-side QR generation |
...
| **Icons** | lucide-react | Lightweight, scalable vector icons (`Sun`, `Moon`, `RefreshCw`, `X`, `CheckCircle2`) |
| **Typography** | Google Fonts | `Inter` (UI elements) & `JetBrains Mono` (high-clarity bin digits) |
| **Hosting & Edge** | Vercel | Global CDN deployment with automated SPA routing & immutable caching |
...
This repository includes a pre-configured `vercel.json` file with SPA rewrite rules and caching policies.
